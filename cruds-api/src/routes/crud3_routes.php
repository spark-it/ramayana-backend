...
include __DIR__ . '/../models/Crud3.php';


//Get a list
$app->get('/crud3', function ($request, $response, $args) {
    $rows = Crud3::all();
    return $response->withJson($rows, 200);
});

//Get specific crud
$app->get('/crud3/{id}', function ($request, $response, $args) {
    $rows = Crud3::find($args['id']);
    return $response->withJson($rows, 200);
});

$app->post('/crud3', function ($request, $response, $args) {
    $response_code = 201;
