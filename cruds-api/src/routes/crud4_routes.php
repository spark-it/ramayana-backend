...
include __DIR__ . '/../models/Crud4.php';


//Get a list
$app->get('/crud4', function ($request, $response, $args) {
    $rows = Crud4::all();
    return $response->withJson($rows, 200);
});

//Get specific crud
$app->get('/crud4/{id}', function ($request, $response, $args) {
    $rows = Crud4::find($args['id']);
    return $response->withJson($rows, 200);
});

$app->post('/crud4', function ($request, $response, $args) {
    $response_code = 201;
