...
$app->get('/admin/sobre/list', function ($request, $response, $args) {
    check_logged($response);
    $rows = Sobre::all();

    $this->renderer->render($response, "/admin/head.phtml", ['base_url' => BASE_URL]);
    $this->renderer->render($response, "/admin/sobre/list.phtml", ['rows' => $rows, 'base_url' => BASE_URL]);
    $this->renderer->render($response, "/admin/foot.phtml", $args);
});

$app->get('/admin/sobre/create', function ($request, $response, $args) {
    check_logged($response);

    $this->renderer->render($response, "/admin/head.phtml", ['base_url' => BASE_URL]);
    $this->renderer->render($response, "/admin/sobre/create.phtml", ['base_url' => BASE_URL]);
    $this->renderer->render($response, "/admin/foot.phtml", $args);
});
...
$app->post('/admin/sobre', function ($request, $response, $args) {
    $response_code = 201;
    ...
    if ($response_code == 400) {
        ...
        if ($fromForm) {
            return $response->withRedirect($base_url . '/admin/sobre/list');
        } else {
...
$app->post('/admin/sobre/edit/{id}', function ($request, $response, $args) {
    $response_code = 201;
...
$app->post('/admin/sobre/delete', function ($request, $response, $args) {
    $parsedBody = $request->getParsedBody();
    $id = $parsedBody['id'];
