...
include __DIR__ . '/../models/Crud1.php';


//Get a list
$app->get('/crud1', function ($request, $response, $args) {
    $rows = Crud1::all();
    return $response->withJson($rows, 200);
});

//Get specific crud
$app->get('/crud1/{id}', function ($request, $response, $args) {
    $rows = Crud1::find($args['id']);
    return $response->withJson($rows, 200);
});

$app->post('/crud1', function ($request, $response, $args) {
    $response_code = 201;
