...
include __DIR__ . '/../models/Crud5.php';


//Get a list
$app->get('/crud5', function ($request, $response, $args) {
    $rows = Crud5::all();
    return $response->withJson($rows, 200);
});

//Get specific crud
$app->get('/crud5/{id}', function ($request, $response, $args) {
    $rows = Crud5::find($args['id']);
    return $response->withJson($rows, 200);
});

$app->post('/crud5', function ($request, $response, $args) {
    $response_code = 201;
